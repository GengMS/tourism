        ...
        <el-form ref="form" :model="user" label-width="0">
          <div class="form-items">
            ...
            <el-row class="form-item form-item-geng">
              <el-col>
                <el-input type="text" v-model="mailCode" maxlength="4" placeholder="验证码"></el-input>
                <el-button
                  @click="getCodeRequest"
                  :disabled="disabled"
                  >{{ num }}</el-button
                >
              </el-col>
...
const backgroundImage = require("../../assets/CoUBYGBhNV-AN-SPADHuD9JYNGM647.png");
export default {
  ...
  created() {
    if (
      this.$cookies.get("registy") == null ||
      this.$cookies.get("registy") <= 0 ||
      this.$cookies.get("registy") == 60
    ) {
      this.$cookies.set("registy", 60);
      this.$cookies.set("registy", 60);
      this.disabled = false;
      this.num = "获取";
    } else if (
  ...
  methods: {
    submitBtn() {
      ...
      this.axios.post(this.$root.loginUrl+"/register/"+this.user.username+"/"+this.user.password+"/"+this.mailCode)
      .then(res => {
        if(res.data){
           this.$message({
              message: "注册成功",
    ...
    getCode() {
      let a = setInterval(() => {
        if (this.$cookies.get("registy") <= 0) {
          this.disabled = false;
          this.num = "获取";
          clearInterval(a);
        } else {
          this.disabled = true;
          this.$cookies.set("registy", this.$cookies.get("registy") - 1);
          this.num = this.$cookies.get("registy");
        }
      }, 1000);
    },
  },
